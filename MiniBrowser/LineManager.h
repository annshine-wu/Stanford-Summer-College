...
private:
    ...
    void deleteTreeHelper(Node* currentNode);
    double contentHeightHelper(Node* current) const;
    void linesInRangeHelper(Node* current, double min, double max, Vector<Line *>& range) const;
    Line* lineAtHelper(double y, Node* current) const;
};
