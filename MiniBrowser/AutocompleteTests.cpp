...
ADD_TEST("Searches should not fails when no words match the prefix") {
    Set<string> toInsert = {
        "Starter",   "Code",    "Long",    "Match",    "Prefix",
        "Sass",    "Fails",  "Search",    "Searching",
    };
    Autocomplete ac;
    for (auto word: toInsert) {
        ac.add(word);
    }
    expect(ac.suggestionsFor("Starters", 100000).isEmpty());
    expect(ac.suggestionsFor("eijiw3", 100000).isEmpty());
    expect(ac.suggestionsFor("e2oo2", 100000).isEmpty());
    expect(ac.suggestionsFor("2201", 100000).isEmpty());
    expect(ac.suggestionsFor("acbsd", 100000).isEmpty());
}

ADD_TEST("Searches should not fail when there prefix is longer than any word within the set") {
    Set<string> toInsert = {
        "Starter",   "Code",    "Long",    "Match",    "Prefix",
        "Sass",    "Fails",  "Search",    "Searching",
    };
    ...
    expect(ac.suggestionsFor("ewrtfyghkjhgfdsfdgfhgjhkjgf", 100000).isEmpty());
}
