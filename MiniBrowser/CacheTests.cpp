...
ADD_TEST("Get function should move requested element to the front"){
    Cache<int> cache(4);
    ...
    expect(cache.returnFront() == 4);
}

ADD_TEST("Cache starts off as empty and does not have any elements in it"){
    Cache<int> cache(5);
    expect(cache.getSize() == 0);
}
