...
ADD_TEST("Hit tests work on the first and last lines.") {
    Vector<Line *> lines = {
        new Line{ 0, 27 }, new Line{ 301, 353 }, new Line { 860, 5309 }
    };

    LineManager manager(lines);
    expect(manager.lineAt(0) == lines[0]);
    expect(manager.lineAt(5309) == lines[2]);
}

ADD_TEST("Find range-queries that are for the first and last lines") {
    Vector<Line *> lines = {
        new Line{ 0, 27 }, new Line{ 301, 353 }, new Line { 860, 5309 }
    };
    ...
    Set<Line *> matches;
    matches.add(lines[0]);
    expect(asSet(manager.linesInRange(-10, 27)) == matches);
    matches.clear();
    matches.add(lines[2]);
    expect(asSet(manager.linesInRange(855, 870)) == matches);
}
